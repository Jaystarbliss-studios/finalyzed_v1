...
-- Fix writer onboarding 409s caused by the globally unique point_transactions.reference.
-- The starter-grant reference must be unique per writer, not globally shared.
create or replace function public.complete_onboarding(
  p_role text, p_full_name text, p_username text, p_avatar_url text, p_phone text,
  p_institution text, p_faculty text, p_department text, p_degree text,
  p_matric_number text, p_graduation_year text, p_bio text,
  p_specialties text[], p_portfolio_url text
) returns jsonb
language plpgsql
security definer
...
declare
  ...
  normalized_username text := nullif(lower(trim(p_username)), '');
  starter_reference text := 'WRITER_STARTER_300_' || uid::text;
begin
  ...
  update public.profiles pr set full_name=nullif(trim(p_full_name),''),username=normalized_username,avatar_url=nullif(trim(p_avatar_url),''),
    role=normalized_role,account_status=case when normalized_role='student' then 'approved'::public.application_status else 'pending'::public.application_status end,
    onboarding_complete=true,updated_at=now() where pr.id=uid;
  if not found then
    insert into public.profiles(id,full_name,username,avatar_url,role,account_status,onboarding_complete)
    values(uid,nullif(trim(p_full_name),''),normalized_username,nullif(trim(p_avatar_url),''),normalized_role,
      case when normalized_role='student' then 'approved' else 'pending' end,true);
  end if;

  insert into public.wallets(user_id,points_balance) values(uid,case when normalized_role='writer' then 300 else 0 end)
  on conflict(user_id) do update set points_balance=case when normalized_role='writer' and public.wallets.points_balance=0 then 300 else public.wallets.points_balance end,updated_at=now();

  if normalized_role='writer' then
    insert into public.point_transactions(user_id,direction,points,reason,reference)
    select uid,'credit',300,'New writer starter grant',starter_reference
    where not exists(select 1 from public.point_transactions pt where pt.user_id=uid and pt.reference=starter_reference);
  end if;
  ...
  if normalized_role='student' then
    ...
    on conflict(user_id) do update set phone=excluded.phone,institution=excluded.institution,faculty=excluded.faculty,department=excluded.department,degree=excluded.degree,matric_number=excluded.matric_number,graduation_year=excluded.graduation_year,updated_at=now();
