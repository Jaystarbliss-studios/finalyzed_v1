...
const label = (key: string) => LABELS[key] ?? key.replace(/([a-z])([A-Z])/g, '$1 $2').replace(/_/g, ' ').replace(/^./, c => c.toUpperCase());
...
const pick = (spec: Specification, ...keys: string[]): unknown => {
...
function Field({ fieldKey, value }: { fieldKey: string; value: unknown }) {
  const isObject = typeof value === 'object' && value !== null;
  return (
    <div className="rounded-xl border border-border bg-background p-3">
      <div className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">{label(fieldKey)}</div>
      {isObject ? (
        <pre className="mt-1.5 max-h-36 overflow-auto whitespace-pre-wrap break-words font-mono text-xs leading-5">{text(value)}</pre>
      ) : (
        <div className="mt-1.5 break-words whitespace-pre-wrap text-sm font-medium">{text(value)}</div>
      )}
    </div>
  );
}
...
export default function ProjectSpecificationSummary({ project, specification }: Props) {
  ...
  const schemaStages = Array.isArray(schema) ? schema.filter((stage): stage is Record<string, unknown> => typeof stage === 'object' && stage !== null).map((stage, index): Stage => ({
  ...
  const fallbackStages: Stage[] = fallbackKeys.map(([title, description, keys]) => ({
    title, description,
    fields: keys.filter(key => answerMap[key] !== undefined || specification?.[key] !== undefined).map(key => ({ key }))
  })).filter(stage => stage.fields.length > 0 || stage.title === 'Project overview');
  ...
  const explicitPoints = pick(specification, 'price_points', 'pricePoints', 'points', 'projectPoints');
  const points = typeof explicitPoints === 'number' && explicitPoints >= 0 ? explicitPoints : typeof project.price_points === 'number' && project.price_points >= 0 ? project.price_points : project.plan.toLowerCase() === 'basic' ? 100 : 150;

  return (
    <section className="bento-card mb-6 overflow-hidden">
      <div className="border-b border-border bg-muted/20 p-5 md:p-6">
        <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
          <div><div className="flex items-center gap-2 text-primary"><FileText className="h-4 w-4" /><span className="mono-label">CONFIRMED SPECIFICATION</span></div><h2 className="mt-2 text-lg font-bold md:text-xl">{title}</h2><p className="mt-1 text-sm text-muted-foreground">The exact requirements attached to this commission.</p>{source && <p className="mt-2 text-xs font-semibold text-primary">Started from template: {text(source)}</p>}</div>
          <div className="flex flex-wrap gap-2"><Status value={status} ok={status.toUpperCase() === 'CONFIRMED'} /><Status value={complete ? 'Complete' : 'Incomplete'} ok={complete} /></div>
        </div>
      </div>
      <div className="p-4 md:p-6">
        <div className="mb-4 grid gap-3 sm:grid-cols-4">
          <div className="rounded-xl border border-border p-3"><div className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Plan</div><div className="mt-1 font-bold">{project.plan}</div></div>
          <div className="rounded-xl border border-border p-3"><div className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Project status</div><div className="mt-1 font-bold">{project.status}</div></div>
          <div className="rounded-xl border border-border p-3"><div className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Confirmation</div><div className="mt-1 font-bold">{confirmed ? 'Confirmed' : 'Pending'}</div></div>
          <div className="rounded-xl border border-primary/20 bg-primary/5 p-3"><div className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Project price</div><div className="mt-1 font-bold text-primary">{points} Points</div></div>
        </div>
        <div className="space-y-2">
          {stages.map((stage, index) => (
            <div key={stage.title + index} className="overflow-hidden rounded-xl border border-border">
              <button type="button" onClick={() => setOpen(open === index ? null : index)} aria-expanded={open === index} className="flex w-full items-center justify-between gap-3 p-4 text-left hover:bg-muted/30">
                <div><span className="text-[10px] font-bold uppercase tracking-wider text-primary">Stage {index + 1}</span><div className="mt-0.5 font-bold">{stage.title}</div></div>
                <ChevronDown className={`h-4 w-4 transition-transform ${open === index ? 'rotate-180' : ''}`} />
              </button>
              {open === index && <div className="border-t border-border p-4"><p className="mb-3 text-xs text-muted-foreground">{stage.description}</p><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{stage.fields.map(field => <Field key={field.key} fieldKey={field.key} value={valueFor(field)} />)}</div></div>}
            </div>
          ))}
        </div>
        {additional.length > 0 && <details className="mt-4 rounded-xl border border-border"><summary className="cursor-pointer px-4 py-3 text-sm font-semibold">Additional stored requirements ({additional.length})</summary><div className="grid gap-3 border-t border-border p-4 sm:grid-cols-2 lg:grid-cols-3">{additional.slice(0, 50).map(([key, value]) => <Field key={key} fieldKey={key} value={value} />)}</div></details>}
        <div className="mt-4 flex items-center gap-2 rounded-xl border border-primary/15 bg-primary/5 p-3 text-xs text-muted-foreground"><ShieldCheck className="h-4 w-4 shrink-0 text-primary" />This view reflects the stored commission specification. Template changes remain part of the submitted requirements.</div>
      </div>
    </section>
  );
}
