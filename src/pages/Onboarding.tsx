...
export default function Onboarding() {
  ...
  return <div className="w-full max-w-5xl mx-auto px-4 pt-6 pb-[calc(2rem+env(safe-area-inset-bottom))] sm:py-10 md:py-16 min-h-[100dvh]">
    ...
    {step===2 && meta && <motion.div initial={{opacity:0,y:12}} animate={{opacity:1,y:0}} className="bento-card p-6 md:p-8 max-w-3xl mx-auto"><button type="button" onClick={()=>setStep(1)} className="flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground mb-6"><ArrowLeft className="w-4 h-4"/> Change role</button><div className="flex items-center gap-4 mb-7"><div className="w-12 h-12 rounded-xl bg-primary/10 flex items-center justify-center"><meta.icon className="w-6 h-6 text-primary"/></div><div><p className="text-xs uppercase tracking-wider font-bold text-primary">Registration</p><h2 className="text-2xl font-bold">{meta.title}</h2></div></div>
      <form onSubmit={handleComplete} className="space-y-5 pb-2"><div className="grid grid-cols-1 md:grid-cols-2 gap-4"><Field label="Full Name" name="fullName" value={formData.fullName} onChange={update} required placeholder="Your legal/full name"/><Field label="Username" name="username" value={formData.username} onChange={update} required placeholder="e.g. johnwrites"/></div><p className="-mt-2 text-xs text-muted-foreground">Your full name is used for official records and administrator review. Every account has a unique public username that students and other users can see.</p><label className="block space-y-2"><span className="text-sm font-medium">Profile photo (optional)</span><input type="file" accept="image/*" onChange={e=>setAvatarFile(e.target.files?.[0]||null)} className="w-full bg-background border border-border rounded-xl px-4 py-3 text-sm"/><span className="text-xs text-muted-foreground">PNG/JPG/WebP, maximum 5 MB. Uploaded securely to Cloudinary.</span></label><div className="grid grid-cols-1 md:grid-cols-2 gap-4"><Field label="Phone Number" name="phone" value={formData.phone} onChange={update} type="tel" required/>{role==='student'?<InstitutionPicker value={formData.institution} onChange={v=>update('institution',v)} required/>:<Field label="Institution / Organization" name="institution" value={formData.institution} onChange={update} required/>}</div>
