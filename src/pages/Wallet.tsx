...
interface WalletRecord { balance_ngn?: number; points_balance?: number; }
interface LedgerItem { id: string; transaction_type?: string; amount_ngn?: number; points?: number; metadata?: any; created_at?: string; }
...
export default function Wallet() {
  ...
  const loadWallet = async () => {
    ...
    setLoading(true);
    try {
      const [{ data: walletRow, error: walletError }, { data: txRows, error: txError }] = await Promise.all([
...
function LedgerRow({ tx, money }: { tx: LedgerItem; money: (v?: number) => string }) { const negative = ['withdrawal_hold', 'fee', 'debit', 'points_to_cash'].includes(String(tx.transaction_type || '').toLowerCase()); const date = tx.created_at ? new Date(tx.created_at).toLocaleString('en-NG') : '—'; return <div className="p-4 flex items-center justify-between gap-4 hover:bg-muted/30 transition-colors"><div className="flex items-center gap-3 min-w-0"><div className={`p-2.5 rounded-full shrink-0 ${negative ? 'bg-red-500/10 text-red-500' : 'bg-primary/10 text-primary'}`}>{negative ? <ArrowUpRight className="w-4 h-4" /> : <ArrowDownLeft className="w-4 h-4" />}</div><div className="min-w-0"><p className="font-semibold text-sm truncate">{tx.metadata?.description || tx.transaction_type || 'Wallet transaction'}</p><p className="text-xs text-muted-foreground mt-0.5">{date}</p></div></div><div className="text-right shrink-0"><p className={`font-bold ${negative ? 'text-red-500' : 'text-primary'}`}>{negative ? '-' : '+'}{money(Math.abs(Number(tx.amount_ngn || 0)))}</p><p className="text-xs text-muted-foreground capitalize">recorded</p></div></div>; }
