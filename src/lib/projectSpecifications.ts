...
function toRow(specification: Record<string, unknown>, ownerId: string, status: 'DRAFT' | 'CONFIRMED', version: number) {
  ...
  return {
    ...
    font: String(specification.font ?? ''),
    font_size: String(specification.fontSize ?? ''),
    line_spacing: String(specification.lineSpacing ?? ''),
    margins: String(specification.margins ?? ''),
    alignment: String(specification.alignment ?? ''),
    page_numbering: String(specification.pageNumbering ?? ''),
    chapter_headings: String(specification.chapterHeadings ?? ''),
    chapter_count: Number(specification.chapterCount ?? 5) || 5,
    standards_bodies: String(specification.standardsBodies ?? ''),
    core_subject_matter: String(specification.coreSubjectMatter ?? ''),
    target_outcome: String(specification.targetOutcome ?? ''),
    real_data: String(specification.realData ?? ''),
    scope: String(specification.scope ?? ''),
    special_instructions: String(specification.specialInstructions ?? ''),
