...
export async function createProjectFromSpecification(specificationId:string,plan:PlanType,priceNgn:number){ const {data,error}=await supabase.rpc('create_project_from_spec',{p_specification_id:specificationId,p_plan:plan,p_price_ngn:priceNgn}); if(error)throw error; return data; }
