...
export async function getAvailableWriters(search=''){ let query=supabase.from('public_profiles').select('*').eq('verified',true).order('ranking_score',{ascending:false}).order('rating',{ascending:false}).order('completed_projects',{ascending:false}); if(search.trim()) query=query.or(`display_name.ilike.%${search.trim()}%,bio.ilike.%${search.trim()}%`); const {data,error}=await query; if(error)throw error; return data??[]; }
